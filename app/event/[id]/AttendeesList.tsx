...
export default function AttendeesList({ eventId }: { eventId: string }) {
    ...
    const fetchAttendees = async () => {
        const { data, error } = await supabase
            ...
            .select('user_email, user_display_name, user_avatar_url, created_at')
            .eq('event_id', eventId)
            ...
            {attendees.length > 0 ? (
                <div className="flex flex-wrap gap-3">
                    {attendees.map((att, idx) => (
                        <div key={idx} className="bg-white/5 border border-white/10 pr-4 pl-1 py-1 rounded-full text-xs text-gray-300 font-medium flex items-center gap-2">
                            <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-brand-blue to-brand-purple flex items-center justify-center text-white font-bold text-[10px] overflow-hidden">
                                {att.user_avatar_url ? (
                                    <img src={att.user_avatar_url} alt="Profil" className="w-full h-full object-cover" />
                                ) : (
